...
export function generateLiveStroke(canvas) {
    ...
    // modificare directă a direcției după voce:
    // - vol = cât de tare
    // - V = cât de repede te schimbi → schimbare de direcție
    // - E = distribuția frecvențelor → oscilații
    // - peak → "explozie" de schimbare
    const force = {
        x: (Math.sin(E * 10 + V * 5) + (Math.random() - 0.5) * V * 2) * (2 + vol * 40),
        y: (Math.cos(E * 12 + V * 3) + (Math.random() - 0.5) * V * 2) * (2 + vol * 40)
    };

    // aplicăm forța pe viteză
    vel.x += force.x * 0.1;
    vel.y += force.y * 0.1;

    // "fricțiune" ca să pară organic
    vel.x *= 0.96;
    vel.y *= 0.96;
    ...
    // bounce elegant dacă iese din ecran
